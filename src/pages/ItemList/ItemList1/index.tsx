...
export const ItemList1 = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setSelectNav(context[0]?.name);
      await axios({
        method: 'get',
        url: `${process.env.REACT_APP_API_URL}/item/list?size=12&page=0&${sort}&categoryName=${context[0]?.name}&itemName=${searchWord.current}`,
      }).then((res) => {
        setItemList(res.data.content);
        setReqData(`${sort}&categoryName=${context[0]?.name}&itemName=${searchWord.current}`);
  ...
  return (
    <>
      <MetaTag
        ...
        url='https://safekingmall.com/itemlist-1'
