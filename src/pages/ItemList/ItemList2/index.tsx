...
export const ItemList2 = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setSelectNav(context[1]?.name);
      await axios({
        method: 'get',
        url: `${process.env.REACT_APP_API_URL}/item/list?size=12&page=0&${sort}&categoryName=${context[1]?.name}&itemName=${searchWord.current}`,
      }).then((res) => {
        setItemList(res.data.content);
        setReqData(`${sort}&categoryName=${context[1]?.name}&itemName=${searchWord.current}`);
  ...
  return (
    <>
      <MetaTag
        ...
        url='https://safekingmall.com/itemlist-2'
        keywords={`안전왕, 안전, 안전관리, 안전사고, 사고예방, ${selectNav}`}
      />
      <S.Container>
        <Header />
        <Nav categoryList={categoryList} selectNav={selectNav} slideNavIdx={1} />
        <Search
          setSort={setSort}
          selectSort={selectSort}
