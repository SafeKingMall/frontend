...
export const ItemList5 = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setSelectNav(context[4]?.name);
      await axios({
        method: 'get',
        url: `${process.env.REACT_APP_API_URL}/item/list?size=12&page=0&${sort}&categoryName=${context[4]?.name}&itemName=${searchWord.current}`,
      }).then((res) => {
        setItemList(res.data.content);
        setReqData(`${sort}&categoryName=${context[4]?.name}&itemName=${searchWord.current}`);
  ...
  return (
    <>
      <MetaTag
        ...
        url='https://safekingmall.com/itemlist-5'
        keywords={`안전왕, 안전, 안전관리, 안전사고, 사고예방, ${selectNav}`}
      />
      <S.Container>
        <Header />
        <Nav categoryList={categoryList} selectNav={selectNav} slideNavIdx={4} />
        <Search
          setSort={setSort}
          selectSort={selectSort}
