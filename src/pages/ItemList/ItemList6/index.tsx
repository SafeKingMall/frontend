...
export const ItemList6 = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setSelectNav(context[5]?.name);
      await axios({
        method: 'get',
        url: `${process.env.REACT_APP_API_URL}/item/list?size=12&page=0&${sort}&categoryName=${context[5]?.name}&itemName=${searchWord.current}`,
      }).then((res) => {
        setItemList(res.data.content);
        setReqData(`${sort}&categoryName=${context[5]?.name}&itemName=${searchWord.current}`);
  ...
  return (
    <>
      <MetaTag
        ...
        url='https://safekingmall.com/itemlist-6'
