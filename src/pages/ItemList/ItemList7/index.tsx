...
export const ItemList7 = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setSelectNav(context[6]?.name);
      await axios({
        method: 'get',
        url: `${process.env.REACT_APP_API_URL}/item/list?size=12&page=0&${sort}&categoryName=${context[6]?.name}&itemName=${searchWord.current}`,
      }).then((res) => {
        setItemList(res.data.content);
        setReqData(`${sort}&categoryName=${context[6]?.name}&itemName=${searchWord.current}`);
  ...
  return (
    <>
      <MetaTag
        ...
        url='https://safekingmall.com/itemlist-7'
        keywords={`안전왕, 안전, 안전관리, 안전사고, 사고예방, ${selectNav}`}
      />
      <S.Container>
        <Header />
        <Nav categoryList={categoryList} selectNav={selectNav} slideNavIdx={6} />
        <Search
          setSort={setSort}
          selectSort={selectSort}
